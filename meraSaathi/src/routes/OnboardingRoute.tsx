...
export function OnboardingRoute({ onComplete }: OnboardingRouteProps) {
  ...
  // Loading Screen Logic — saves onboarding data to MongoDB
  useEffect(() => {
    if (step === 5) {
      const texts = [
      ...
      const interval = setInterval(() => {
        ...
        if (currentProgress >= 100) {
          ...
          // Save onboarding data to MongoDB via the backend API
          if (!hasSaved) {
            hasSaved = true;
            updateProfile({
              age: age || undefined,
  ...
  }, [step, email, age, interests, goals, onComplete, updateProfile]);
